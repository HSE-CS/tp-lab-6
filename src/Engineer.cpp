...
void Engineer::calc() {
    payment = calcBudgetPart(0.1)
            + calcProAdditions() + calcBonus() + calcBase();
}
...
Programmer::Programmer(unsigned int id, std::string name,
                       unsigned int salary) : Engineer(id, name, salary) {
    this->position = programmer;
}

void Programmer::printInfo() {
    std::cout << "ID: " << id << std::endl << "Name: "
    << name << std::endl << "Position: Programmer" << std::endl
