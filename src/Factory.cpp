...
std::vector <Employee*> Factory::makeStaff(std::string projectsPath, std::string staffPath){
  ...
  while (line != ""){
    ...
    std::string name = line.substr(line.find(';') + 1, line.rfind(';') - line.find(';') - 1);
  ...
  while (line != ""){
    ...
    if (position == "DRIVER"){
      ...
      employees.push_back(new Programmer(id, name, PROGRAMMER, salary, proj_pointer));
      ...
      employees.push_back(new TeamLeader(id, name, TEAM_LEADER, salary, proj_pointer));
      ...
      employees.push_back(new ProjectManager(id, name, PROJECT_MANAGER, proj_pointer));
    } else if (position == "SENIOR_MANAGER"){
      for (Project* p : projects) {
            p->numOfWorkers++;
      }
      employees.push_back(new SeniorManager(id, name, PROJECT_MANAGER, projects));
    }
    std::getline(fin, line);
  }
  fin.close();
