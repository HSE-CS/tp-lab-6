...
Employee *makeWorker( int id, const std::string &name,  int workTime,
    ...
    switch (position) {
        case PROJECT_MANAGER:
            return new ProjectManager(id, name, workTime, position, std::move(projects));
        case TEAM_LEADER:
            return new TeamLeader(id, name, workTime, salary, position, projects[0]);
        case TESTER:
            return new Tester(id, name, workTime, salary, position, projects[0]);
        case PROGRAMMER:
            return new Programmer(id, name, workTime, salary, position, projects[0]);
        case SENIOR_MANAGER:
            return new SeniorManager(id, name, workTime, position, projects);
        case DRIVER:
            return new Driver(id, name, workTime, salary, position);
        case CLEANER:
            return new Cleaner(id, name, workTime, salary, position);
        default:
            return nullptr;
    }
}
...
std::vector<Employee *> Factory::makeStaff() {
    std::ifstream projects_file("/home/victor/CLionProjects/tp-lab-6/projects.csv");
    std::vector<Project *> projects;
    int project_id, count_workers, budget = 0;
    std::string project_name;

    std::ifstream workers_file("/home/victor/CLionProjects/tp-lab-6/workers.csv");
