...
Personal::Personal(unsigned int id_, const std::string& name_, Position pos,
    int salary_)
...
Driver::Driver(unsigned int id_, const std::string& name_, Position pos,
    int salary_)
    : Personal(id_, name_, pos, salary_) {}

int Driver::calcBonus() { return 500; }

void Driver::calc() { payment = calcBase(salary, worktime) + calcBonus(); }

Cleaner::Cleaner(unsigned int id_, const std::string& name_, Position pos,
    int salary_)
    : Personal(id_, name_, pos, salary_) {}

int Cleaner::calcBonus() { return 0; }

void Cleaner::calc() { payment = calcBase(salary, worktime) + calcBonus(); }
