...
TeamLeader::TeamLeader(unsigned int id, const std::string& name)
            : Programmer(id, name) {
    this->id = id;
...
void TeamLeader::calc() {
    this->payment = calcBase(this->salaryPerHour, this->worktime) + calcHeads()
                    + calcProAdditions() + calcBudgetPart();
}
void TeamLeader::printInfo() {
    ...
    if (this->linkToProject) {
        std::cout << "Number of subordinates " <<
                this->linkToProject->getNumberOfMembers() << std::endl;
    } else {
