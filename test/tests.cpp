...
TEST(EmployeesTest, test1) {
  ...
  Programmer employee(111, "TestName", Position::PROGRAMMER, 10000,
   project, 0.1);
...
TEST(EmployeesTest, test4) {
  Project* project = new Project(1, 10000, 0);
  TeamLeader employee(111, "TestName", Position::TEAMLEADER, 10000, project,
   0.1);
  employee.SetWorkTime(1);
  employee.Calc();
...
TEST(EmployeesTest, test5) {
  Project* project = new Project(1, 10000, 0);
  ProjectManager employee(111, "TestName", Position::PROJECTMANAGER, project,
   0.1);
  employee.SetWorkTime(1);
  employee.Calc();
...
TEST(EmployeesTest, test11) {
  Cleaner employee(111, "TestName", Position::CLEANER, 10000);
  EXPECT_EQ("TestName", employee.GetName());
}
...
TEST(EmployeesTest, test13) {
  Cleaner employee(111, "TestName", Position::CLEANER, 10000);
  EXPECT_EQ("Cleaner", employee.GetPosition());
}

TEST(EmployeesTest, test14) {
  Project* project = new Project(1, 10000, 111);
  std::vector<Project*> projects = {project};
  SeniorManager employee(111, "TestName", Position::SENIORMANAGER, projects);
  EXPECT_EQ("Senior Manager", employee.GetPosition());
}

TEST(EmployeesTest, test15) {
  Project* project = new Project(1, 10000, 0);
  EXPECT_EQ(10000, project->GetBudget());
}
