...
TEST(Test, test3) {
  Cleaner* temp = new Cleaner("23", "tamara", 200.0, Position(CLEANER));
  EXPECT_EQ("CLEANER", temp->getPosition());
}
...
TEST(Test, test5) {
  ...
  Programmer* temp = new Programmer("23", "tamara", 200.0, Position(CLEANER), pr);
  temp->setWorkTime(30);
...
TEST(Test, test11) {
  Project* pr = new Project("G4", 10000);
  TeamLeader* temp = new TeamLeader("23", "tamara", 200.0, Position(CLEANER), pr);
  temp->setWorkTime(30);
  temp->calc();
